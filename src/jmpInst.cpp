...
JmpInst::JmpInst(std::string name, std::shared_ptr<std::vector<std::shared_ptr<Token>>> tokens) : Instruction(name, tokens)
{
    if (operands->size() == 1)
    {
        ...
        {
            if ((*operands)[0].getType() == Operand::Type::JMP_SYMBOL_DIR || (*operands)[0].getType() == Operand::Type::JMP_SYMBOL_IMM || (*operands)[0].getType() == Operand::Type::JMP_SYMBOL_REG_OFF || (*operands)[0].getType() == Operand::Type::SYMBOL_DIR || (*operands)[0].getType() == Operand::Type::PC_RELATIVE)
            {
                std::shared_ptr<Symbol> sym = SymTable::instance().getSymbol((*operands)[0].getName());
